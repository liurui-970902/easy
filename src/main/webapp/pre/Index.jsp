<%@page import="com.liurui.pojo.EasybuyUser"%>
...
<body>
	...
	<%	
		...
		if(null == request.getAttribute("list")){
			out.print("<script type=\"text/javascript\"> location.href=\""+path+"/spring/IndexServlet/index\" </script>");
		}
	 %>
		...
		<div class="menu">
												...
												<div class="zj_l_c">
													...
													<c:forEach var="list3" items="${list3 }" varStatus="index2">
														<c:if test="${list3.parentId==list2.id }">
															<a
																href="${ctx}/spring/ProductServlet/queryProductList3?category=${list3.id}">${list3.name }</a>|
													</c:if>
													</c:forEach>
												</div>
											</c:if>
			...
			<ul class="menu_r">
				...
				<c:forEach items="${list}" var="temp">
					...
					<c:choose>
						<c:when test="${ not empty sessionScope.easybuyUserLogin }">
							<li><a href="${ctx}/spring/ProductServlet/queryProductList?category=${temp.id}&menu=${temp.id}&userId=${sessionScope.easybuyUserLogin.id}" <c:if test="${menu==temp.id}"> class="now" </c:if>>${temp.name}</a></li>
						</c:when>
						<c:otherwise>
							<li><a href="${ctx}/spring/ProductServlet/queryProductList?category=${temp.id}&menu=${temp.id}" <c:if test="${menu==temp.id}"> class="now" </c:if>>${temp.name}</a></li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
			</ul>
	...
	<div class="i_bg bg_color">
		<div class="i_ban_bg">
			...
			<div class="banner">
				<div class="top_slide_wrap">
					<ul class="slide_box bxslider">
						<li><a href="${ctx}/spring/ProductServlet/queryProductDeatil?id=771">
						<img src="${ctx}/statics/images/a1.jpg" width="740"
							height="401" /></a></li>
						<li><a href="${ctx}/spring/ProductServlet/queryProductDeatil?id=772">
						<img src="${ctx}/statics/images/a2.jpg" width="740"
							height="401" /></a></li>
						<li><a href="${ctx}/spring/ProductServlet/queryProductDeatil?id=773">
						<img src="${ctx}/statics/images/a3.jpg" width="740"
							height="401" /></a></li>
						<li><a href="${ctx}/spring/ProductServlet/queryProductDeatil?id=774">						
			...
			<div class="inews">
				<div class="news_t">
					<span class="fr"><a
						href="${ctx }/spring/AdminNewsServlet/queryNewsList">更多 ></a></span>新闻资讯
				</div>
				<ul>
					<c:forEach var="newsList" items="${newsList }"
						...
						<c:if test="${newsIndex.count<=5 }">
							<li><span> <c:if test="${newsIndex.count%2==1 }">[ 特惠 ]</c:if>
								...
								href="${ctx }/spring/AdminNewsServlet/newsDeatil?newsId=${newsList.id }">${newsList.title }</a></li>
						</c:if>
		...
		<div class="content mar_10">
			<div class="h_l_img">
				<div class="img">
					<a href="${ctx}/spring/ProductServlet/queryProductDeatil?id=740">
									...
									<c:forEach var="productList" items="${productList }"
										...
										<li class="featureBox">
											<div class="box">
												...
												<div class="imgbg">
													<a
														href="${ctx}/spring/ProductServlet/queryProductDeatil?id=${productList.id}&userId=${sessionScope.easybuyUserLogin == null?'':sessionScope.easybuyUserLogin.id}"><img
														src="${ctx}/files/${productList.fileName }" width="160"
														height="136" /></a>
												</div>
												<div class="name">
													<a
														href="${ctx}/spring/ProductServlet/queryProductDeatil?id=${productList.id}">
			...
			<div class="i_t mar_10">
				<span class="floor_num">${index.count }F</span> <span class="fl">${list.name }</span><span
					class="i_mores fr"> <c:forEach var="list2" items="${list2 }"
						...
						<c:if test="${list2.parentId==list.id }">
							<a
								href="${ctx}/spring/ProductServlet/queryProductList2?category=${list2.id}">${list2.name }</a>
						</c:if>
					</c:forEach>
				</span>
			</div>
							...
							<ul class="imgs" id="actor${index.count }">
								<c:forEach var="productList" items="${productList }"
									...
									<c:if test="${productList.categoryLevel1==list.id }">
										<li><a
											href="${ctx}/spring/ProductServlet/queryProductDeatil?id=${productList.id}"><img
						...
						<c:forEach var="productList" items="${productList }"
							...
							<c:if test="${productList.categoryLevel1==list.id }">
								<li>
									<div class="name">
										<a
											href="${ctx}/spring/ProductServlet/queryProductDeatil?id=${productList.id}">${productList.name }</a>
									</div>
									<div class="price">
										<font>￥<span>${productList.price }</span></font> &nbsp; 26R
									</div>
