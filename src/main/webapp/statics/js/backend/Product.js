...
function updateByProductId(id, currentPage) {
	...
	$(".m_right").load(contextPath + "/spring/ProductServlet/getProduct .m_right>*", ise);
}
...
function checkProduct() {
	...
	$.ajax({
		url : contextPath + "/spring/ProductServlet/getImgs",
		method : "post",
		data : formData,
		contentType : false,
		processData : false,
		success : function(data) {
			...
			if(result.message=="添加成功!") { //添加成功!
				...
				//返回商品显示列表
				$(".m_right").load(contextPath+"/spring/ProductServlet/index .m_right>*");
			}else if(result.message=="修改成功!"){ //修改成功
				mizhu.toast("操作成功",1500);
				//得到在哪一个修改
				var currentPage = $("#currentPage").val();
				//返回商品显示列表
				$(".m_right").load(contextPath+"/spring/ProductServlet/index?currentPage="+currentPage+" .m_right>*");
			}else{
...
function change1() {
	...
	$("#productCategoryLevel2").load(contextPath + "/spring/ProductCategoryService/getProductCategoryTwo #productCategoryLevel2>*",item, leve2);
}
...
function leve2() {
	...
	$("#productCategoryLevel3").load(
			contextPath + "/spring/ProductCategoryService/getProductCategoryThree #productCategoryLevel3>*",
			item);
}
