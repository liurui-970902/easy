 ...
 * 
 * @param id
 * @returnsPrintUtil.write(result, response);
 */
function delProduct(id) {
	$.ajax({
		url : contextPath + "/spring/ProductServlet/deleteById",
		method : "post",
		data : {
			"id" : id
		},
		success : function(jsonStr) {
			...
			if (result.status == 1) {
				...
				$(".m_right").load(contextPath+ "/spring/ProductServlet/index .m_right>*",ise);
			}
		}
	});
...
function s_btn(thisId, currentPage) {
	...
	if (identityCode == "" || identityCode == null) {
		...
		//判断用户名是否存在
		$.ajax({
			url		:		contextPath+"/spring/EasybuyUserServlet/loginNameCount?name="+name,
			method	:		"post",
			success	:		function(data){
				...
				if(json.status == -1 && name != oldloginName){
					...
					var item = {
						...
						$.post(contextPath + "/spring/EasybuyUserServlet/modify", item,
							function(data) {
								var result = eval("(" + data + ")");
								if (result.message == "添加成功!") {  //添加成功!
									mizhu.toast('操作成功！', 2000);
									//添加成功后跳回用户显示页面
									$(".m_right").load( contextPath + "/spring/EasybuyUserServlet/user .m_right>*");
								}else if(result.message == "更新成功!"){ //修改成功!
									mizhu.toast('更新成功!',1200);
									//修改成功后跳回用户显示页面
									$(".m_right").load( contextPath + "/EasybuyUserServlet/user .m_right>*", "currentPage="+currentPage);
								}
						});
				}
			}
...
function UpdatePwd(){
	...
	if($oldpwd == ""){
		...
		//执行修改,使用ajax
		$.ajax({
			url			:		contextPath+"/spring/EasybuyUserServlet/modifyPassWord",
			method		:		"post",
			...
			success		:		function(data){
				...
				if(json.status == 1){
					//alert(json.message);
					//跳转至登录页面
					location.href=contextPath+"/spring/EasybuyUserServlet/forgetPwd";
				}
			}
...
function checkOrder(orderId,currentPage,isall,userId){
	mizhu.confirm('温馨提醒', '确认收货嘛？','确认', '取消', function(flag){
		//确认收货
		$.ajax({
			url			:		contextPath+"/spring/EasybuyOrderServlet/confirmOrder",
			method		:		"post",
			data		:		{
				orderId	: orderId,
			currentPage : currentPage
			...
			success     : 		function(data){
				...
				if(json.status == 1){
					...
					//成功,跳回订单页面
					if(isall == "1"){ //操作的是我的订单
						$(".m_right").load(contextPath+"/spring/EasybuyOrderServlet/index?userId="+userId+"&currentPage="+currentPage+" .m_right>*");
					}else{//操作的是全部订单
						$(".m_right").load(contextPath+"/spring/EasybuyOrderServlet/queryAllOrder?currentPage="+currentPage+" .m_right>*");
					}
				}else{
					mizhu.toast(json.message,1200);
				}
