 ...
 */
 function delCategory(type,id,currentPage) {
	 mizhu.confirm('温馨提醒', '要删除该分类吗？',"确定","取消", function(flag) {
			if(flag) {
				$.ajax({
					"url" : contextPath + "/spring/ProductCategoryService/delCategory",
					"data" : {
						"type":type,
						"id" : id
					},
					...
					"success" : function(data) {
						...
						if (result.status == 1) {
							...
							$(".m_right").load(contextPath+"/spring/ProductCategoryService/category .m_right>*",ise);
						}
					}
				});
...
function toAddProductCategory(currentPage) {
	$("#AddProductCategory").load(contextPath + "/spring/ProductCategoryService/AddProductCategory","currentPage="+currentPage);
}
/**
 * 分类级别change事件！
 */
...
function queryProductCategoryList(onedd) {
	...
	$("#productCategoryLevel2").load(contextPath+ "/spring/ProductCategoryService/addCategoryLevel2 #productCategoryLevel2>*",item);
}
...
function saveinsert() {
	...
	$.post(contextPath + "/spring/ProductCategoryService/addCategory", item, function(data) {
		var result = eval("(" + data + ")");
		// 添加成功与否弹出信息！
		mizhu.alert('操作提醒', result.message);
		if (result.status == 1) {
			...
			$(".m_right").load(contextPath+"/spring/ProductCategoryService .m_right>*",ise);
		}
	});
}
/**
...
function saveOrUpdate() {
	...
	$.post(contextPath + "/spring/ProductCategoryService/insertCategory", item, function(data) {
		var result = eval("(" + data + ")");
		// 更新成功与否弹出信息！
		mizhu.alert('操作提醒',result.message);
		if (result.status == 1) {
...
function update(tid,currentPage) {
	...
	$("#AddProductCategory").load(contextPath + "/spring/ProductCategoryService/upProductCategory",item);
} 
